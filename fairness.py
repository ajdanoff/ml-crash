...
class MyLayer(Layer):
    def __init__(self, fun=tf.stack, **kwargs):
    ...
    def call(self, x):
        ...
        for key in sorted(x.keys()):
            values.append(tf.cast(x[key], tf.float64))
        return self.fun(values, axis=-1)

x = MyLayer(fun=tf.concat)(inputs)
...
normalizer.adapt(MyLayer()(dict(features)))
...
print(base_model_analysis.sample(5))
